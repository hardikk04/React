...
function Details() {
  ...
  const handleDelete = (productId) => {
    const filteredData = products.filter((p) => p.id != productId);
        ...
        <div className="product-content h-full w-[45%] px-4 py-8 flex flex-col">
          ...
          <div>
            <div className="flex gap-4">
              <Link
                to={`/edit/${product.id}`}
                className="w-1/2 py-5 mt-[2vw] text-center inline-block border-[1px] border-blue-500 border-solid text-blue-500"
              >
                Edit
              </Link>
              <button
                onClick={() => handleDelete(product.id)}
                className="w-1/2 py-5 mt-[2vw] text-center inline-block border-[1px] border-red-500 border-solid text-red-500"
              >
                Delete
              </button>
            </div>
